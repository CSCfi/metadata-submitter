...
class ConfigTests(TestCase):
    ...
    @mock.patch.dict(os.environ, {"MONGO_SSL_CLIENT_KEY": "client.key"})
    @mock.patch.dict(os.environ, {"MONGO_SSL_CLIENT_CERT": "client.crt"})
    ...
    def test_all_values(self):
        ...
        _params = "ceva?tls=true&tlsCAFile=ca.crt&ssl_keyfile=client.key&ssl_certfile=client.crt&authSource=admin"
        self.assertEqual(
            url,
    ...
    def test_ssl_no_db_value(self):
        ...
        url, _ = set_conf()
        _params = "ceva?tls=true&tlsCAFile=ca.crt&ssl_keyfile=client.key&ssl_certfile=client.crt"
        self.assertEqual(
            url,
            f"mongodb://admin:admin@localhost:27017/{_params}",
        )

...
from metadata_backend.api.processors.xml import XmlIdentifierPaths, XmlObjectConfig, \
    XmlSchemaPath, XmlReferenceIdentifierPaths, XmlObjectProcessor, XmlObjectIdentifier, \
    BP_ANNOTATION_SCHEMA, BP_IMAGE_SCHEMA, XmlDocumentsProcessor, BP_SAMPLE_SCHEMA, BP_DATASET_SCHEMA, \
    BP_OBSERVATION_SCHEMA, BP_OBSERVER_SCHEMA, BP_POLICY_SCHEMA, BP_STAINING_SCHEMA, BP_ORGANISATION_SCHEMA, \
    BP_REMS_SCHEMA, BP_XML_OBJECT_CONFIG, XmlFileDocumentsProcessor, XmlIdentifierPath, BP_SAMPLE_BIOLOGICAL_BEING_PATH, \
    ...
    BP_ORGANISATION_PATH, BP_REMS_PATH, BP_LANDING_PAGE_SCHEMA, BP_LANDING_PAGE_PATH, XmlDocumentProcessor, \
...
async def test_with_element_existing():
    ...
    config = XmlObjectConfig(
        ...
        identifier_paths=[
            XmlIdentifierPaths(schema_type=schema_type,
                               root_path=root_path,
                               paths=[
                                   XmlIdentifierPath(
                                       name_path="Name",
        ...
        ref_identifier_paths=[
            XmlReferenceIdentifierPaths(schema_type=schema_type,
    ...
    ref_1 = XmlObjectIdentifier(schema_type=ref_schema_type, root_path=ref_root_path, name=ref_name_1, id=ref_id_1)
    ref_2 = XmlObjectIdentifier(schema_type=ref_schema_type, root_path=ref_root_path, name=ref_name_2, id=ref_id_2)
...
def assert_identifier(processor: XmlDocumentsProcessor,
...
async def test_bp_submission_1():
    ...
    dataset_dir = TEST_FILES_DIR / "xml" / "bp" / "submission_1"
    ...
    # Annotation
    assert_identifier(processor, BP_ANNOTATION_SCHEMA_AND_PATH, annotation_name, None)
    ...
    assert_ref(processor, BP_ANNOTATION_SCHEMA_AND_PATH, annotation_name, BP_IMAGE_SCHEMA_AND_PATH,
               image_name, None)
    # Dataset
    assert_identifier(processor, BP_DATASET_SCHEMA_AND_PATH, dataset_name, None)
    ...
    # Image
    assert_identifier(processor, BP_IMAGE_SCHEMA_AND_PATH, image_name, None)
    ...
    # Observation
    assert_identifier(processor, BP_OBSERVATION_SCHEMA_AND_PATH, observation_name, None)
    ...
    assert_ref(processor, BP_OBSERVATION_SCHEMA_AND_PATH, observation_name, BP_OBSERVER_SCHEMA_AND_PATH,
               observer_name, None)
    # Observer
    assert_identifier(processor, BP_OBSERVER_SCHEMA_AND_PATH, observer_name, None)
    assert_ref_length(processor, BP_OBSERVER_SCHEMA_AND_PATH, observer_name, 0)
    # Policy
    assert_identifier(processor, BP_POLICY_SCHEMA_AND_PATH, policy_name, None)
    ...
    assert_ref(processor, BP_POLICY_SCHEMA_AND_PATH, policy_name, BP_DATASET_SCHEMA_AND_PATH, dataset_name,
               None)
    # Sample
    assert_identifier(processor, BP_SAMPLE_BIOLOGICAL_BEING_SCHEMA_AND_PATH, sample_biological_being_name,
                      None)
    assert_identifier(processor, BP_SAMPLE_CASE_SCHEMA_AND_PATH, sample_case_name, None)
    assert_identifier(processor, BP_SAMPLE_SPECIMEN_SCHEMA_AND_PATH, sample_specimen_name, None)
    assert_identifier(processor, BP_SAMPLE_BLOCK_SCHEMA_AND_PATH, sample_block_name, None)
    assert_identifier(processor, BP_SAMPLE_SLIDE_SCHEMA_AND_PATH, sample_slide_name, None)
    ...
    assert_ref(processor, BP_SAMPLE_SLIDE_SCHEMA_AND_PATH, sample_slide_name, BP_STAINING_SCHEMA_AND_PATH,
               staining_name, None)
    # Staining
    assert_identifier(processor, BP_STAINING_SCHEMA_AND_PATH, staining_name, None)
    assert_ref_length(processor, BP_STAINING_SCHEMA_AND_PATH, staining_name, 0)
    # Landing page
    assert_identifier(processor, BP_LANDING_PAGE_SCHEMA_AND_PATH, landing_page_name, None)
    ...
    assert_ref(processor, BP_LANDING_PAGE_SCHEMA_AND_PATH, landing_page_name, BP_DATASET_SCHEMA_AND_PATH,
               dataset_name, None)
    # Organisation
    assert_identifier(processor, BP_ORGANISATION_SCHEMA_AND_PATH, organisation_name, None)
    ...
    assert_ref(processor, BP_ORGANISATION_SCHEMA_AND_PATH, organisation_name, BP_DATASET_SCHEMA_AND_PATH,
               dataset_name, None)
    # Rems
    assert_identifier(processor, BP_REMS_SCHEMA_AND_PATH, rems_name, None)
    ...
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_ANNOTATION_SCHEMA, root_path=BP_ANNOTATION_PATH, name=annotation_name,
                            id=annotation_id))
    # Dataset
    dataset_id = f"dataset_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_DATASET_SCHEMA, root_path=BP_DATASET_PATH, name=dataset_name, id=dataset_id))
    # Image
    image_id = f"image_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_IMAGE_SCHEMA, root_path=BP_IMAGE_PATH, name=image_name, id=image_id))
    # Observation
    observation_id = f"observation_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_OBSERVATION_SCHEMA, root_path=BP_OBSERVATION_PATH, name=observation_name,
                            id=observation_id))
    # Observer
    observer_id = f"observer_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_OBSERVER_SCHEMA, root_path=BP_OBSERVER_PATH, name=observer_name,
                            id=observer_id))
    # Policy
    policy_id = f"policy_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_POLICY_SCHEMA, root_path=BP_POLICY_PATH, name=policy_name, id=policy_id))
    # Staining
    staining_id = f"staining_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_STAINING_SCHEMA, root_path=BP_STAINING_PATH, name=staining_name,
                            id=staining_id))
    # Landing page
    landing_page_id = f"landing_page_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_LANDING_PAGE_SCHEMA, root_path=BP_LANDING_PAGE_PATH, name=landing_page_name,
                            id=landing_page_id))
    # Sample
    ...
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_SAMPLE_SCHEMA, root_path=BP_SAMPLE_BIOLOGICAL_BEING_PATH,
                            name=sample_biological_being_name, id=sample_biological_being_id))
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_SAMPLE_SCHEMA, root_path=BP_SAMPLE_CASE_PATH,
                            name=sample_case_name, id=sample_case_id))
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_SAMPLE_SCHEMA, root_path=BP_SAMPLE_SPECIMEN_PATH,
                            name=sample_specimen_name, id=sample_specimen_id))
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_SAMPLE_SCHEMA, root_path=BP_SAMPLE_BLOCK_PATH,
                            name=sample_block_name, id=sample_block_id))
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_SAMPLE_SCHEMA, root_path=BP_SAMPLE_SLIDE_PATH,
                            name=sample_slide_name, id=sample_slide_id))
    # Organisation
    organisation_id = f"organisation_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_ORGANISATION_SCHEMA, root_path=BP_ORGANISATION_PATH, name=organisation_name,
                            id=organisation_id))
    # Rems
    rems_id = f"rems_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=BP_REMS_SCHEMA, root_path=BP_REMS_PATH, name=rems_name, id=rems_id))

    # Assert accessioned state
    #

    # Annotation
    assert_identifier(processor, BP_ANNOTATION_SCHEMA_AND_PATH, annotation_name, annotation_id)
    assert_ref_length(processor, BP_ANNOTATION_SCHEMA_AND_PATH, annotation_name, 1)
    assert_ref(processor, BP_ANNOTATION_SCHEMA_AND_PATH, annotation_name, BP_IMAGE_SCHEMA_AND_PATH, image_name,
               image_id)
    # Dataset
    assert_identifier(processor, BP_DATASET_SCHEMA_AND_PATH, dataset_name, dataset_id)
    assert_ref_length(processor, BP_DATASET_SCHEMA_AND_PATH, dataset_name, 3)
    ...
    # Image
    assert_identifier(processor, BP_IMAGE_SCHEMA_AND_PATH, image_name, image_id)
    assert_ref_length(processor, BP_IMAGE_SCHEMA_AND_PATH, image_name, 1)
    assert_ref(processor, BP_IMAGE_SCHEMA_AND_PATH, image_name, BP_SAMPLE_SLIDE_SCHEMA_AND_PATH,
               sample_slide_name, sample_slide_id)
    # Observation
    assert_identifier(processor, BP_OBSERVATION_SCHEMA_AND_PATH, observation_name, observation_id)
    assert_ref_length(processor, BP_OBSERVATION_SCHEMA_AND_PATH, observation_name, 2)
    assert_ref(processor, BP_OBSERVATION_SCHEMA_AND_PATH, observation_name, BP_ANNOTATION_SCHEMA_AND_PATH,
               annotation_name, annotation_id)
    assert_ref(processor, BP_OBSERVATION_SCHEMA_AND_PATH, observation_name, BP_OBSERVER_SCHEMA_AND_PATH,
    ...
    # Observer
    assert_identifier(processor, BP_OBSERVER_SCHEMA_AND_PATH, observer_name, observer_id)
    assert_ref_length(processor, BP_OBSERVER_SCHEMA_AND_PATH, observer_name, 0)
    # Policy
    assert_identifier(processor, BP_POLICY_SCHEMA_AND_PATH, policy_name, policy_id)
    assert_ref_length(processor, BP_POLICY_SCHEMA_AND_PATH, policy_name, 1)
    ...
    assert_identifier(processor, BP_SAMPLE_CASE_SCHEMA_AND_PATH, sample_case_name, sample_case_id)
    assert_identifier(processor, BP_SAMPLE_SPECIMEN_SCHEMA_AND_PATH, sample_specimen_name, sample_specimen_id)
    assert_identifier(processor, BP_SAMPLE_BLOCK_SCHEMA_AND_PATH, sample_block_name, sample_block_id)
    assert_identifier(processor, BP_SAMPLE_SLIDE_SCHEMA_AND_PATH, sample_slide_name, sample_slide_id)
    assert_ref_length(processor, BP_SAMPLE_BIOLOGICAL_BEING_SCHEMA_AND_PATH, sample_biological_being_name, 0)
    assert_ref_length(processor, BP_SAMPLE_CASE_SCHEMA_AND_PATH, sample_case_name, 1)
    ...
    # Staining
    assert_identifier(processor, BP_STAINING_SCHEMA_AND_PATH, staining_name, staining_id)
    assert_ref_length(processor, BP_STAINING_SCHEMA_AND_PATH, staining_name, 0)
    # Landing page
    assert_identifier(processor, BP_LANDING_PAGE_SCHEMA_AND_PATH, landing_page_name, landing_page_id)
    assert_ref_length(processor, BP_LANDING_PAGE_SCHEMA_AND_PATH, landing_page_name, 1)
    assert_ref(processor, BP_LANDING_PAGE_SCHEMA_AND_PATH, landing_page_name, BP_DATASET_SCHEMA_AND_PATH,
               dataset_name, dataset_id)
    # Organisation
    assert_identifier(processor, BP_ORGANISATION_SCHEMA_AND_PATH, organisation_name, organisation_id)
    assert_ref_length(processor, BP_ORGANISATION_SCHEMA_AND_PATH, organisation_name, 1)
    assert_ref(processor, BP_ORGANISATION_SCHEMA_AND_PATH, organisation_name, BP_DATASET_SCHEMA_AND_PATH,
               dataset_name, dataset_id)
    # Rems
    assert_identifier(processor, BP_REMS_SCHEMA_AND_PATH, rems_name, rems_id)
    assert_ref_length(processor, BP_REMS_SCHEMA_AND_PATH, rems_name, 1)
...
async def test_fega_submission_1():
    ...
    dataset_dir = TEST_FILES_DIR / "xml" / "fega" / "submission_1"
    ...
    # Analysis
    assert_identifier(processor, FEGA_ANALYSIS_SCHEMA_AND_PATH, analysis_name, None)
    ...
    assert_ref(processor, FEGA_ANALYSIS_SCHEMA_AND_PATH, analysis_name, FEGA_RUN_SCHEMA_AND_PATH,
               run_name, None)
    # Dac
    assert_identifier(processor, FEGA_DAC_SCHEMA_AND_PATH, dac_name, None)
    assert_ref_length(processor, FEGA_DAC_SCHEMA_AND_PATH, dac_name, 0)
    # Dataset
    assert_identifier(processor, FEGA_DATASET_SCHEMA_AND_PATH, dataset_name, None)
    ...
    assert_ref(processor, FEGA_DATASET_SCHEMA_AND_PATH, dataset_name, FEGA_POLICY_SCHEMA_AND_PATH,
               policy_name, None)
    # Experiment
    assert_identifier(processor, FEGA_EXPERIMENT_SCHEMA_AND_PATH, experiment_name, None)
    ...
    assert_ref(processor, FEGA_EXPERIMENT_SCHEMA_AND_PATH, experiment_name, FEGA_SAMPLE_SCHEMA_AND_PATH,
               sample_name, None)
    # Policy
    assert_identifier(processor, FEGA_POLICY_SCHEMA_AND_PATH, policy_name, None)
    ...
    assert_ref(processor, FEGA_POLICY_SCHEMA_AND_PATH, policy_name, FEGA_DAC_SCHEMA_AND_PATH, dac_name,
               None)
    # Run
    assert_identifier(processor, FEGA_RUN_SCHEMA_AND_PATH, run_name, None)
    ...
    assert_ref(processor, FEGA_RUN_SCHEMA_AND_PATH, run_name, FEGA_EXPERIMENT_SCHEMA_AND_PATH, experiment_name,
               None)
    # Sample
    assert_identifier(processor, FEGA_SAMPLE_SCHEMA_AND_PATH, sample_name, None)
    assert_ref_length(processor, FEGA_SAMPLE_SCHEMA_AND_PATH, sample_name, 0)
    # Study
    assert_identifier(processor, FEGA_STUDY_SCHEMA_AND_PATH, study_name, None)
    assert_ref_length(processor, FEGA_STUDY_SCHEMA_AND_PATH, study_name, 0)
    # Submission
    assert_identifier(processor, FEGA_SUBMISSION_SCHEMA_AND_PATH, submission_name, None)
    ...
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_ANALYSIS_SCHEMA, root_path=FEGA_ANALYSIS_PATH, name=analysis_name,
                            id=analysis_id))
    # Dac
    dac_id = f"dac_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_DAC_SCHEMA, root_path=FEGA_DAC_PATH, name=dac_name, id=dac_id))
    # Dataset
    dataset_id = f"dataset_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_DATASET_SCHEMA, root_path=FEGA_DATASET_PATH, name=dataset_name,
                            id=dataset_id))
    # Experiment
    experiment_id = f"experiment_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_EXPERIMENT_SCHEMA, root_path=FEGA_EXPERIMENT_PATH, name=experiment_name,
                            id=experiment_id))
    # Policy
    policy_id = f"policy_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_POLICY_SCHEMA, root_path=FEGA_POLICY_PATH, name=policy_name, id=policy_id))
    # Run
    run_id = f"run_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_RUN_SCHEMA, root_path=FEGA_RUN_PATH, name=run_name, id=run_id))
    # Sample
    sample_id = f"sample_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_SAMPLE_SCHEMA, root_path=FEGA_SAMPLE_PATH, name=sample_name, id=sample_id))
    # Study
    study_id = f"study_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_STUDY_SCHEMA, root_path=FEGA_STUDY_PATH, name=study_name, id=study_id))
    # Submission
    submission_id = f"submission_{str(uuid.uuid4())}"
    processor.set_xml_object_id(
        XmlObjectIdentifier(schema_type=FEGA_SUBMISSION_SCHEMA, root_path=FEGA_SUBMISSION_PATH, name=submission_name,
                            id=submission_id))
    ...
    # Analysis
    assert_identifier(processor, FEGA_ANALYSIS_SCHEMA_AND_PATH, analysis_name, analysis_id)
    assert_ref_length(processor, FEGA_ANALYSIS_SCHEMA_AND_PATH, analysis_name, 4)
    assert_ref(processor, FEGA_ANALYSIS_SCHEMA_AND_PATH, analysis_name, FEGA_STUDY_SCHEMA_AND_PATH,
               study_name, study_id)
    assert_ref(processor, FEGA_ANALYSIS_SCHEMA_AND_PATH, analysis_name, FEGA_SAMPLE_SCHEMA_AND_PATH,
    ...
    assert_ref(processor, FEGA_ANALYSIS_SCHEMA_AND_PATH, analysis_name, FEGA_RUN_SCHEMA_AND_PATH,
               run_name, run_id)
    # Dac
    assert_identifier(processor, FEGA_DAC_SCHEMA_AND_PATH, dac_name, dac_id)
    assert_ref_length(processor, FEGA_DAC_SCHEMA_AND_PATH, dac_name, 0)
    # Dataset
    assert_identifier(processor, FEGA_DATASET_SCHEMA_AND_PATH, dataset_name, dataset_id)
    assert_ref_length(processor, FEGA_DATASET_SCHEMA_AND_PATH, dataset_name, 3)
    assert_ref(processor, FEGA_DATASET_SCHEMA_AND_PATH, dataset_name, FEGA_RUN_SCHEMA_AND_PATH, run_name,
               run_id)
    assert_ref(processor, FEGA_DATASET_SCHEMA_AND_PATH, dataset_name, FEGA_ANALYSIS_SCHEMA_AND_PATH,
    ...
    # Experiment
    assert_identifier(processor, FEGA_EXPERIMENT_SCHEMA_AND_PATH, experiment_name, experiment_id)
    assert_ref_length(processor, FEGA_EXPERIMENT_SCHEMA_AND_PATH, experiment_name, 2)
    assert_ref(processor, FEGA_EXPERIMENT_SCHEMA_AND_PATH, experiment_name, FEGA_STUDY_SCHEMA_AND_PATH, study_name,
               study_id)
    assert_ref(processor, FEGA_EXPERIMENT_SCHEMA_AND_PATH, experiment_name, FEGA_SAMPLE_SCHEMA_AND_PATH,
               sample_name, sample_id)
    # Policy
    assert_identifier(processor, FEGA_POLICY_SCHEMA_AND_PATH, policy_name, policy_id)
    assert_ref_length(processor, FEGA_POLICY_SCHEMA_AND_PATH, policy_name, 1)
    assert_ref(processor, FEGA_POLICY_SCHEMA_AND_PATH, policy_name, FEGA_DAC_SCHEMA_AND_PATH, dac_name,
               dac_id)
    # Run
    assert_identifier(processor, FEGA_RUN_SCHEMA_AND_PATH, run_name, run_id)
    assert_ref_length(processor, FEGA_RUN_SCHEMA_AND_PATH, run_name, 1)
    assert_ref(processor, FEGA_RUN_SCHEMA_AND_PATH, run_name, FEGA_EXPERIMENT_SCHEMA_AND_PATH, experiment_name,
               experiment_id)
    # Sample
    assert_identifier(processor, FEGA_SAMPLE_SCHEMA_AND_PATH, sample_name, sample_id)
    assert_ref_length(processor, FEGA_SAMPLE_SCHEMA_AND_PATH, sample_name, 0)
    # Study
    assert_identifier(processor, FEGA_STUDY_SCHEMA_AND_PATH, study_name, study_id)
    assert_ref_length(processor, FEGA_STUDY_SCHEMA_AND_PATH, study_name, 0)
    # Submission
    assert_identifier(processor, FEGA_SUBMISSION_SCHEMA_AND_PATH, submission_name, submission_id)
    assert_ref_length(processor, FEGA_SUBMISSION_SCHEMA_AND_PATH, submission_name, 0)

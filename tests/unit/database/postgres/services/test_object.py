...
async def test_get_documents(
    ...
    async with transaction(session_factory, requires_new=True, rollback_new=True) as session:
        ...
        draft_objects = [create_object_entity(submission.submission_id, schema=schema, is_draft=True) for _ in range(3)]
        for obj in draft_objects:
            await object_repository.add_object(obj)

        # Get draft documents.
        documents = [
            document async for document in object_service.get_documents(submission.submission_id, schema, is_draft=True)
        ]
        xml_documents = [
            document
            async for document in object_service.get_xml_documents(submission.submission_id, schema, is_draft=True)
        ]
        ...
        documents = [
            document
            async for document in object_service.get_documents(submission.submission_id, schema, is_draft=False)
        ]
        xml_documents = [
            document
            async for document in object_service.get_xml_documents(submission.submission_id, schema, is_draft=False)
        ]
        ...
        finalized_objects = [
            create_object_entity(submission.submission_id, schema=schema, is_draft=False) for _ in range(3)
        ]
...
async def test_count_objects(
    ...
    async with transaction(session_factory, requires_new=True, rollback_new=True) as session:
        ...
        # Create draft objects.
        objects = [create_object_entity(submission.submission_id, schema=schema, is_draft=is_draft) for _ in range(3)]
        for obj in objects:
            await object_repository.add_object(obj)

...
from metadata_backend.database.postgres.repository import session
...
async def test_add_and_get_submission(
    submission_repository: SubmissionRepository, submission_service: SubmissionService
):
...
async def test_get_submissions(submission_repository: SubmissionRepository, submission_service: SubmissionService):
    project_id = f"project_{uuid.uuid4()}"
...
async def test_get_project_id(submission_repository: SubmissionRepository, submission_service: SubmissionService):
    submission = create_submission_entity()
    await submission_repository.add_submission(submission)
    assert await submission_service.get_project_id(submission.submission_id) == submission.project_id


async def test_get_workflow(submission_repository: SubmissionRepository, submission_service: SubmissionService):
    submission = create_submission_entity()
    await submission_repository.add_submission(submission)
    assert await submission_service.get_workflow(submission.submission_id) == submission.workflow


async def test_get_bucket(submission_repository: SubmissionRepository, submission_service: SubmissionService):
    submission = create_submission_entity()
    await submission_repository.add_submission(submission)
...
async def test_delete_submission(submission_repository: SubmissionRepository, submission_service: SubmissionService):
    submission = create_submission_entity()
    await submission_repository.add_submission(submission)
    assert await submission_service.is_submission_by_id(submission.submission_id)

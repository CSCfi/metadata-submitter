...
async def test_add_get_delete_object(
    submission_repository: SubmissionRepository, object_repository: ObjectRepository
) -> None:
...
async def test_get_and_count_objects(
    ...
    results = [
        obj
        async for obj in object_repository.get_objects(submission_id=first_submission_id, object_type=first_object_type)
    ]
    assert len(results) == 1

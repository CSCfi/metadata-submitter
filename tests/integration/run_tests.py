...
# === Helper functions ===
async def get_user_data(client_logged_in):
    """Get current logged in user's data model.

    :param client_logged_in: HTTP client in which request call is made
    """
    async with client_logged_in.get(f"{users_url}/current") as resp:
...
async def get_object(client_logged_in, schema, accession_id):
    """Get one metadata object within client, returns object's data.
...
async def post_object(client_logged_in, schema, submission_id, filename):
    """Post one metadata object within client, returns accessionId.
...
async def post_multi_object(client_logged_in, schema, submission_id, filename):
    """Post metadata objects from one file within client, returns response body (json).
...
async def post_object_expect_status(client_logged_in, schema, submission_id, filename, status):
    """Post one metadata object within client, returns accessionId.
...
async def post_object_json(client_logged_in, schema, submission_id, filename):
    """Post & put one metadata object within client, returns accessionId.
...
async def delete_object(client_logged_in, schema, accession_id):
    """Delete metadata object within client.

    :param client_logged_in: HTTP client in which request call is made
    :param schema: name of the schema (submission) used for testing
    :param accession_id: id of the object
    """
    async with client_logged_in.delete(f"{objects_url}/{schema}/{accession_id}") as resp:
        LOG.debug(f"Deleting object {accession_id} from {schema}")
        assert resp.status == 204, f"HTTP Status code error, got {resp.status}"


async def post_draft(client_logged_in, schema, submission_id, filename):
    """Post one draft metadata object within client, returns accessionId.
...
async def post_draft_json(client_logged_in, schema, submission_id, filename):
    """Post & put one metadata object within client, returns accessionId.
...
async def get_draft(client_logged_in, schema, draft_id, expected_status=200):
...
async def put_draft(client_logged_in, schema, draft_id, update_filename):
    """Put one metadata object within client, returns accessionId.

    :param client_logged_in: HTTP client in which request call is made
    :param schema: name of the schema (submission) used for testing
    :param draft_id: id of the draft
    :param update_filename: name of the file used to use for updating data.
    :return: asclient id of updated draft
    """
    request_data = await create_request_json_data(schema, update_filename)
    async with client_logged_in.put(f"{drafts_url}/{schema}/{draft_id}", data=request_data) as resp:
...
async def put_object_json(client_logged_in, schema, accession_id, update_filename):
    """Put one metadata object within client, returns accessionId.
    ...
    :param accession_id: id of the object
    :param update_filename: name of the file used to use for updating data.
    :return: asclient id of updated object
    """
    request_data = await create_request_json_data(schema, update_filename)
    async with client_logged_in.put(f"{objects_url}/{schema}/{accession_id}", data=request_data) as resp:
        LOG.debug(f"Try to replace object in {schema}")
        assert resp.status == 415, f"HTTP Status code error, got {resp.status}"


async def patch_object_json(client_logged_in, schema, accession_id, update_filename):
    """Patch one metadata object within client, returns accessionId.
    ...
    """
    request_data = await create_request_json_data(schema, update_filename)
    async with client_logged_in.patch(f"{objects_url}/{schema}/{accession_id}", data=request_data) as resp:
...
async def put_object_xml(client_logged_in, schema, accession_id, update_filename):
    """Put one metadata object within client, returns accessionId.
...
async def patch_draft(client_logged_in, schema, draft_id, update_filename):
    """Patch one metadata object within client, return accessionId.
    ...
    """
    request_data = await create_request_json_data(schema, update_filename)
    async with client_logged_in.patch(f"{drafts_url}/{schema}/{draft_id}", data=request_data) as resp:
...
async def delete_draft(client_logged_in, schema, draft_id):
    """Delete metadata object within client.
    ...
    :param draft_id: id of the draft
    """
    async with client_logged_in.delete(f"{drafts_url}/{schema}/{draft_id}") as resp:
        LOG.debug(f"Deleting draft object {draft_id} from {schema}")
        assert resp.status == 204, f"HTTP Status code error, got {resp.status}"


async def post_template_json(client_logged_in, schema, filename, project_id):
    """Post one metadata object within client, returns accessionId.
    ...
    async with client_logged_in.post(f"{templates_url}/{schema}", data=request_data) as resp:
...
async def get_templates(client_logged_in, project_id):
...
async def get_template(client_logged_in, schema, template_id):
    """Get and return a drafted metadata object.
    ...
    """
    async with client_logged_in.get(f"{templates_url}/{schema}/{template_id}") as resp:
...
async def patch_template(client_logged_in, schema, template_id, update_filename):
    """Patch one metadata object within client, return accessionId.
    ...
    """
    request_data = await create_request_json_data(schema, update_filename)
    async with client_logged_in.patch(f"{templates_url}/{schema}/{template_id}", data=request_data) as resp:
        LOG.debug(f"Update draft object in {schema}")
        assert resp.status == 200, f"HTTP Status code error, got {resp.status}"
        ans_put = await resp.json()
...
async def delete_template(client_logged_in, schema, template_id):
    """Delete metadata object within client.
    ...
    :param template_id: id of the draft
    """
    async with client_logged_in.delete(f"{templates_url}/{schema}/{template_id}") as resp:
        LOG.debug(f"Deleting template object {template_id} from {schema}")
        assert resp.status == 204, f"HTTP Status code error, got {resp.status}"


async def post_submission(client_logged_in, data):
    """Post one object submission within client, returns submissionId.

    :param client_logged_in: HTTP client in which request call is made
    :param data: data used to update the submission
    """
    async with client_logged_in.post(f"{submissions_url}", data=json.dumps(data)) as resp:
        ans = await resp.json()
...
async def patch_submission(client_logged_in, submission_id, data):
    """Patch one object submission within client, return submissionId.

    :param client_logged_in: HTTP client in which request call is made
    :param submission_id: id of the submission
    :param data: JSON object to use in PATCH call
    """
    async with client_logged_in.patch(f"{submissions_url}/{submission_id}", data=json.dumps(data)) as resp:
...
async def publish_submission(client_logged_in, submission_id):
    """Publish one object submission within client, return submissionId.
...
async def delete_submission(client_logged_in, submission_id):
    """Delete object submission within client.
...
async def delete_submission_publish(client_logged_in, submission_id):
    """Delete object submission within client.
...
async def put_submission_doi(client_logged_in, submission_id, data):
    """Put doi into submission within client, returns submissionId.
    ...
    """
    async with client_logged_in.put(f"{submissions_url}/{submission_id}/doi", data=data) as resp:
        ans = await resp.json()
        assert resp.status == 200, f"HTTP Status code error {resp.status} {ans}"
...
async def put_submission_dac(client_logged_in, submission_id, data):
    """Put DAC into submission within client, returns submissionId.
    ...
    :returns: Submission id for the submission inserted to database
    """
    async with client_logged_in.put(f"{submissions_url}/{submission_id}/dac", data=data) as resp:
        ans = await resp.json()
        assert resp.status == 200, f"HTTP Status code error {resp.status} {ans}"
...
async def delete_objects_metax_id(client_logged_in, database, collection, accession_id, metax_id):
    ...
    try:
        await client_logged_in.delete(f"{metax_url}/{metax_id}")
    except Exception as e:
        LOG.error(f"Object deletion from mocked Metax failed due to {str(e)}")


async def delete_user(client_logged_in, user_id):
    """Delete user object within client.

    :param client_logged_in: HTTP client in which request call is made
    :param user_id: id of the user (current)
    """
    async with client_logged_in.delete(f"{users_url}/{user_id}") as resp:
...
async def check_submissions_object_patch(
    client_logged_in, submission_id, schema, accession_id, title, filename, draft=False
):
...
class TestObjects:
    ...
    async def test_crud_works(self, client_logged_in, submission_id):
    ...
    async def test_crud_with_multi_xml(self, client_logged_in, submission_id):
        ...
        data = await post_multi_object(client_logged_in, _schema, submission_id, _filename)
        for item in data:
            items.append(item)
    ...
    async def test_csv(self, client_logged_in, submission_id):
        ...
        samples = await post_object(client_logged_in, _schema, submission_id, _filename)
        ...
        async with client_logged_in.get(f"{objects_url}/{_schema}/{first_sample}") as resp:
            ...
            res = await resp.json()
            title = res.get("title", "")
        await check_submissions_object_patch(client_logged_in, submission_id, _schema, samples, title, _filename)
        ...
        await post_object_expect_status(client_logged_in, _schema, submission_id, _filename, 400)

        _filename = "EGA_sample_w_issue.csv"
        # status should be 201 but we expect 3 rows, as the CSV has 4 rows one of which is empty
        samples_2 = await post_object_expect_status(client_logged_in, _schema, submission_id, _filename, 201)
...
class TestObjectsJsonXml:
    ...
    async def test_put_objects(self, client_logged_in, submission_id):
        """Test PUT reqs.
...
class TestMetax:
    """Test Metax integration."""

    async def test_metax_crud_with_xml(self, client_logged_in, submission_id):
        ...
        for schema, filename, update_filename in {
            ...
            accession_id, _ = await post_object(client_logged_in, schema, submission_id, filename)
        ...
        for object in ids:
            ...
            async with client_logged_in.get(f"{metax_url}/{metax_id}", auth=auth) as metax_resp:
    ...
    async def test_metax_crud_with_json(self, client_logged_in, submission_id):
        ...
        for schema, filename, update_filename in {
            ...
            accession_id = await post_object_json(client_logged_in, schema, submission_id, filename)
    ...
    async def test_metax_id_not_updated_on_patch(self, client_logged_in, submission_id):
    ...
    async def test_metax_publish_dataset(self, client_logged_in, submission_id):
        ...
        for schema, object_id, metax_id in objects:
            ...
            async with client_logged_in.get(f"{metax_url}/{metax_id}") as metax_resp:
                assert metax_resp.status == 200, f"HTTP Status code error, got {metax_resp.status}"
                metax_res = await metax_resp.json()
        ...
        for _, _, metax_id in objects:
            ...
            await client_logged_in.delete(f"{metax_url}/{metax_id}", params={"test": "true"})


class TestMetaxMissingId:
    """Test publication fails with missing metax ID."""

    async def test_metax_publish_dataset_with_missing_metax_id(self, client_logged_in, database, submission_id):
...
class TestSubmissionOperations:
    ...
    # async def test_bpdataset_gets_doi(self, client_logged_in, project_id, submission_id):
...
async def test_minimal_json_publication(client_logged_in, submission_id):
...
async def test_minimal_json_publication_rems(client_logged_in, submission_id):
    """Test minimal publication workflow with json submissions.
...
async def test_schemas_endpoint(client_logged_in):

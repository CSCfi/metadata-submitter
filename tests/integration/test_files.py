...
class TestFiles:
    ...
    project_id = "swift-project"

    async def test_list_buckets(self, client_logged_in, s3_manager):
        """Test listing buckets in a project."""
        response = await client_logged_in.get(f"{files_url}/{self.project_id}/buckets")
    ...
    async def test_list_files_in_bucket(self, client_logged_in, project_id, s3_manager):
        ...
        bucket_name = "bucket1"
        response = await client_logged_in.get(f"{files_url}/{self.project_id}/buckets/{bucket_name}/files")
        assert response.status == 400
        resp = await response.json()
        assert resp["detail"] == f"Bucket {bucket_name} is not accessible in project {self.project_id}."
    ...
    async def test_grant_access_to_bucket(self, client_logged_in, s3_manager):
        """Test creating and reading bucket policies of a bucket in a project."""
        bucket_name = "bucket1"
        response = await client_logged_in.put(f"{files_url}/{self.project_id}/buckets/{bucket_name}")
        assert response.status == 400
        resp = await response.json()
        assert resp["detail"] == "The specified bucket does not exist"

        await s3_manager.add_bucket(bucket_name)
        response = await client_logged_in.head(f"{files_url}/{self.project_id}/buckets/{bucket_name}")
        assert response.status == 400

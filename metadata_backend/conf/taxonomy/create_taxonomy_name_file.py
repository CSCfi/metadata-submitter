...
class Taxonomy:
    ...
    def create_name_taxonomy(self, input_file: Path, output_file: Path) -> Path:
        ...
        param input_file: path to dump file
        param output_file: path to json file
        ...
        for line in self._parse_file(input_file):
            ...
            # add genbank common name. Will be overwritten with common name if available
                ...
                if tax_id in data:
                    ...
                    data[tax_id] = {common: name}
        created_file = self._create_file(data, output_file)
        return created_file

    def _create_file(self, data: dict[str, Any], output_path: Path) -> Path:
